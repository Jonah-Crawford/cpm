    ...
    match rows {
        Ok(rows) => {
            let packages: Vec<PackageListItem> = rows
                .into_iter()
                .map(|(name, description, author, latest, published_at)| PackageListItem {
                    name,
                    description,
                    author,
                    latest,
...
async fn list_versions(State(state): State<AppState>, Path(name): Path<String>) -> impl IntoResponse {
    ...
    match rows {
        Ok(rows) if rows.is_empty() => (StatusCode::NOT_FOUND, "Package not found").into_response(),
        Ok(rows) => {
...
async fn get_package(State(state): State<AppState>, Path(name): Path<String>) -> impl IntoResponse {
    ...
    match row {
        Ok(Some((name, latest, url, sha256))) => Json(PackageResponse {
            name,
            latest,
            url,
            sha256,
...
async fn publish_package(
    ...
    if auth != format!("Bearer {}", state.publish_token) {
        return Err((StatusCode::UNAUTHORIZED, "Invalid publish token".to_string()));
    }

    if pkg.name.trim().is_empty() || pkg.version.trim().is_empty() {
        return Err((StatusCode::BAD_REQUEST, "Package name and version are required".to_string()));
    }
    ...
    match result {
        ...
        Err(sqlx::Error::Database(db_err)) if db_err.is_unique_violation() => {
            Err((StatusCode::CONFLICT, format!("{} v{} already exists", pkg.name, pkg.version)))
        }
...
async fn main() {
    ...
    let publish_token = std::env::var("CPM_PUBLISH_TOKEN").expect("FATAL: CPM_PUBLISH_TOKEN environment variable must be set");
    ...
    let app = Router::new()
        ...
        .with_state(AppState { db, publish_token });
